...
module.exports.GetGoogleFinanceData = function(req, res) {
	...
	function getQuotesInArray(ticker, interval, period, exchange) {
			request(
			{
				uri: 'https://www.google.com/finance/getprices?q=' + ticker + '&x=' + exchange + '&i=' + interval + '&p=' + period + '&f=d,c,v,k,o,h,l&df=cpct',
				method: 'GET'
			}, 
	...
	if (ticker in StoredQuotes) {
		...
		getQuotesInArray(ticker, interval, period, exchange);
	}
}
...
// Get mutual fund list from database
module.exports.GetMutualFundList = function(req, res) {
	model.Companies.findAll({
		order: 'companyid',
		where: {
			exchange : 'MUTF_CA'
		}
	}).then(function(mutualFunds) {
